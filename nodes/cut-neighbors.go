...
func (is *infoSetT) cutNeighborsAndShortenAlongEdges(baseFaceIdx faceIndexT, e1EV, e2EV edgeVectorT) {
	if e1EV.fromVertIdx == e2EV.fromVertIdx {
		is.cutNeighborsAndShortenAlongTwoEdges(e1EV, e2EV)
		return
	}
...
func (is *infoSetT) cutNeighborsAndShortenAlongTwoEdges(e1EV, e2EV edgeVectorT) {
	// log.Printf("cutNeighborsAndShortenAlongTwoEdges(e1EV=%v, e2EV=%v)", e1EV.edge, e2EV.edge)
	...
	amount := e1EV.length
	// log.Printf("cnasate: amount=%v, e1EV=%v", amount, e1EV)
	// 	m := is.faceInfo.m
